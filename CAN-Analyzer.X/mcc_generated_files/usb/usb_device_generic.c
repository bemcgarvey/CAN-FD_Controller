 ...
 *******************************************************************/
void USBCheckVendorRequest(void) {
    if (SetupPkt.bmRequestType == 0b01000001) {
        //Vendor interface OUT request
        if (SetupPkt.bRequest == SET_HARDWARE_CONFIG) {
            USBEP0Receive(ctrlBufferOut, SetupPkt.wLength, &GotHardwareConfig);
            USBDeferStatusStage();
        } else if (SetupPkt.bRequest == SET_FILTER) {
            clearFilter = false;
            filterNumber = SetupPkt.wValue;
            USBEP0Receive(ctrlBufferOut, SetupPkt.wLength, &GotFilter);
            USBDeferStatusStage();
        } else if (SetupPkt.bRequest == CLEAR_ERRORS) {
            status.errorFlags = 0;
            status.systemErrors = 0;
            USBEP0Receive(ctrlBufferOut, SetupPkt.wLength, NULL);
        } else if (SetupPkt.bRequest == SET_FIFO_LENGTHS) {
            USBEP0Receive(ctrlBufferOut, SetupPkt.wLength, &GotFIFOLengths);
            USBDeferStatusStage();
        } else if (SetupPkt.bRequest == CLEAR_FILTER) {
            clearFilter = true;
            filterNumber = SetupPkt.wValue;
            USBEP0Receive(ctrlBufferOut, SetupPkt.wLength, &GotFilter);
            USBDeferStatusStage();
    ...
    {
        if (SetupPkt.bRequest == GET_STATUS) {
            USBEP0SendRAMPtr((uint8_t *)&status, sizeof(StatusInfo), USB_EP0_INCLUDE_ZERO);
        } else if (SetupPkt.bRequest == GET_HARDWARE_INFO) {
            USBEP0SendRAMPtr((uint8_t *)&hardware, sizeof(HardwareInfo), USB_EP0_INCLUDE_ZERO);
        } else if (SetupPkt.bRequest == GET_VERSION) {
           USBEP0SendROMPtr((const uint8_t *) &FirmwareVersion, 2, USB_EP0_ROM | USB_EP0_INCLUDE_ZERO); 
        }  else if (SetupPkt.bRequest == GET_MS_DESCRIPTOR) {
            //Check if the host is requesting an MS feature descriptor
            //Figure out which descriptor is being requested
